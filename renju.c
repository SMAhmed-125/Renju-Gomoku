...
* This file executes a new or unfinished renju game
 ...
 * The main function of the renju game style option.
...
int main( int argc, char *argv[] ) 
{
    ...
    char src_path[DEFAULT_STRING_LENGTH] = "renju";
    ...
    if ( argc == 1 ) {
        Game = game_create( 15, GAME_RENJU );
        game_loop( Game );
        return EXIT_SUCCESS;
    }
    ...
    if ( argc > 1 ) {
        for( int i = 0; i < argc; i++ ) {
            ...
            if ( strcmp( "-o", argv[i] ) == 0 ) {
                ...
                Game = game_create( size, GAME_RENJU );
                game_loop( Game );
                b_flag = true;
